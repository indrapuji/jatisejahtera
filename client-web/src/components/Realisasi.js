import React from 'react';
import {useRouter} from 'next/router';
import Image from 'next/image';
import {home, heart, education} from '@assets/ornamen';
import {motion} from 'framer-motion';

function Realisasi() {
  const router = useRouter();
  const dataRealisasi = [
    {
      image: education,
      title: 'Pendidikan',
      linkTo: '/data/pendidikan',
    },
    {
      image: home,
      title: 'Perumahan',
      linkTo: '/data/perumahan',
    },
    {
      image: heart,
      title: 'Kesehatan',
      linkTo: '/data/kesehatan',
    },
  ];

  const intoVarian = {
    hide: {
      opacity: 0,
      scale: 0.5,
    },
    show: {
      opacity: 1,
      transition: {
        duration: 1,
      },
      scale: 1,
    },
  };
  return (
    <div className='px-4 py-16 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20'>
      <div className='max-w-xl mb-10 md:mx-auto sm:text-center lg:max-w-2xl md:mb-12'>
        <h2 className='mb-12 font-sans text-3xl font-bold leading-none text-gray-900 sm:text-4xl md:mx-auto text-center'>
          <span className='relative inline-block'>
            <svg
              viewBox='0 0 52 24'
              fill='currentColor'
              className='absolute top-0 left-0 z-0 hidden w-32 -mt-8 -ml-20 text-blue-gray-100 lg:w-32 lg:-ml-28 lg:-mt-10 sm:block'
            >
              <defs>
                <pattern id='903f4a9e-7ac3-441c-9613-04c15fcc0a14' x='0' y='0' width='.135' height='.30'>
                  <circle cx='1' cy='1' r='.7' />
                </pattern>
              </defs>
              <rect fill='url(#903f4a9e-7ac3-441c-9613-04c15fcc0a14)' width='52' height='24' />
            </svg>
            <span className='relative'>Data</span>
          </span>{' '}
          realisasi
        </h2>
        {/* <p className='text-base text-gray-700 md:text-lg'>
          Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque rem aperiam, eaque ipsa
          quae.
        </p> */}
      </div>
      <div className='flex justify-around'>
        {dataRealisasi.map((item, index) => (
          <div className='text-center cursor-pointer' key={index} onClick={() => router.push(item.linkTo)}>
            <motion.div
              className='flex items-center justify-center w-16 h-16 mx-auto mb-4 rounded-full bg-indigo-50 sm:w-24 sm:h-24'
              initial='hide'
              whileInView='show'
              exit='hide'
              variants={intoVarian}
            >
              <Image
                src={item.image}
                className='w-12 h-12 text-deep-purple-accent-400 sm:w-16 sm:h-16'
                alt='realisasi'
                width={500}
                height={500}
              />
            </motion.div>
            <h6 className='mb-2 font-semibold leading-5'>{item.title}</h6>
          </div>
        ))}
      </div>
    </div>
  );
}

export default Realisasi;
