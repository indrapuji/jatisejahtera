import React, {useState, useEffect} from 'react';
import Link from 'next/link';
import _Nav from '../routes';
import Logo from '@assets/images/logo.png';
import Image from 'next/image';
import {motion} from 'framer-motion';
import {RiCloseCircleFill} from 'react-icons/ri';

const Navigation = () => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const [slideNav, setSlideNav] = useState(true);
  const [user, setUser] = useState(false);
  const [notifOff, setNotifOff] = useState();

  function changeBackground() {
    if (window.scrollY >= 50) {
      setSlideNav(false);
    } else {
      setSlideNav(true);
    }
  }

  useEffect(() => {
    window.addEventListener('scroll', changeBackground);
    if (localStorage.token) {
      setUser(true);
      setNotifOff(localStorage.status === 'false' ? false : true);
    } else {
      setUser(false);
      setNotifOff(localStorage.status === 'false' ? false : true);
    }

    console.log('status', localStorage.status);
  }, []);

  const notification = () => {
    setNotifOff(true);
  };

  return (
    <div>
      {!notifOff && (
        <div className='relative bg-yellow-100 px-4 py-3 text-white md:block'>
          <p className='text-center text-sm text-red-500 font-medium'>Segera lakukan update data</p>
          <div className='absolute  right-0 -bottom-4'>
            <RiCloseCircleFill color='red' size='30' onClick={() => notification()} />
          </div>
        </div>
      )}

      <div
        className={`px-4 py-5 mx-auto sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 sticky top-0 z-50 ${
          slideNav ? '' : 'bg-white'
        }`}
      >
        <div className='relative flex items-center justify-between'>
          <motion.div
            initial={{opacity: 0, scale: 0.5}}
            animate={{opacity: 1, scale: 1}}
            transition={{
              duration: 0.3,
              ease: [0, 0.71, 0.2, 1.01],
              scale: {
                type: 'spring',
                damping: 5,
                stiffness: 100,
                restDelta: 0.001,
              },
            }}
          >
            <Link href='/' className='inline-flex items-center'>
              <Image className='w-10' src={Logo} alt='logo' />
              <span className='ml-2 text-xl font-bold tracking-wide text-gray-800 uppercase'>JATI SEJAHTERA</span>
            </Link>
          </motion.div>
          <ul className='flex items-center hidden space-x-8 lg:flex'>
            {_Nav.map((item, index) => (
              <li key={index}>
                <Link
                  href={item.linkTo}
                  aria-label='Our product'
                  title='Our product'
                  className='font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400'
                >
                  {item.label}
                </Link>
              </li>
            ))}
          </ul>
          {user ? (
            <ul className='flex items-center hidden space-x-8 lg:flex'>
              <li>
                <a
                  href='/user'
                  className='font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400'
                  aria-label='Login'
                  title='Login'
                >
                  User
                </a>
              </li>
            </ul>
          ) : (
            <ul className='flex items-center hidden space-x-8 lg:flex'>
              <li>
                <a
                  href='/login'
                  className='font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400'
                  aria-label='Login'
                  title='Login'
                >
                  Login
                </a>
              </li>
            </ul>
          )}
          <div className='lg:hidden'>
            <button
              aria-label='Open Menu'
              title='Open Menu'
              className='p-2 -mr-1 transition duration-200 rounded focus:outline-none focus:shadow-outline hover:bg-deep-purple-50 focus:bg-deep-purple-50'
              onClick={() => setIsMenuOpen(true)}
            >
              <svg className='w-5 text-gray-600' viewBox='0 0 24 24'>
                <path fill='currentColor' d='M23,13H1c-0.6,0-1-0.4-1-1s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,13,23,13z' />
                <path fill='currentColor' d='M23,6H1C0.4,6,0,5.6,0,5s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,6,23,6z' />
                <path fill='currentColor' d='M23,20H1c-0.6,0-1-0.4-1-1s0.4-1,1-1h22c0.6,0,1,0.4,1,1S23.6,20,23,20z' />
              </svg>
            </button>
            {isMenuOpen && (
              <div className='absolute top-0 left-0 w-full'>
                <div className='p-5 bg-white border rounded shadow-sm'>
                  <div className='flex items-center justify-between mb-4'>
                    <div>
                      <Link href='/' className='inline-flex items-center'>
                        <Image className='w-10' src={Logo} alt='logo' />
                        <span className='ml-2 text-xl font-bold tracking-wide text-gray-800 uppercase'>
                          JATI SEJAHTERA
                        </span>
                      </Link>
                    </div>
                    <div>
                      <button
                        aria-label='Close Menu'
                        title='Close Menu'
                        className='p-2 -mt-2 -mr-2 transition duration-200 rounded hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline'
                        onClick={() => setIsMenuOpen(false)}
                      >
                        <svg className='w-5 text-gray-600' viewBox='0 0 24 24'>
                          <path
                            fill='currentColor'
                            d='M19.7,4.3c-0.4-0.4-1-0.4-1.4,0L12,10.6L5.7,4.3c-0.4-0.4-1-0.4-1.4,0s-0.4,1,0,1.4l6.3,6.3l-6.3,6.3 c-0.4,0.4-0.4,1,0,1.4C4.5,19.9,4.7,20,5,20s0.5-0.1,0.7-0.3l6.3-6.3l6.3,6.3c0.2,0.2,0.5,0.3,0.7,0.3s0.5-0.1,0.7-0.3 c0.4-0.4,0.4-1,0-1.4L13.4,12l6.3-6.3C20.1,5.3,20.1,4.7,19.7,4.3z'
                          />
                        </svg>
                      </button>
                    </div>
                  </div>
                  <nav>
                    <ul className='space-y-4'>
                      {_Nav.map((item, index) => (
                        <li key={index}>
                          <Link
                            href={item.linkTo}
                            aria-label='Our product'
                            title='Our product'
                            className='font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400'
                          >
                            {item.label}
                          </Link>
                        </li>
                      ))}

                      {user ? (
                        <li>
                          <a
                            href='/user'
                            className='font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400'
                            aria-label='Login'
                            title='User'
                          >
                            User
                          </a>
                        </li>
                      ) : (
                        <li>
                          <a
                            href='/login'
                            className='font-medium tracking-wide text-gray-700 transition-colors duration-200 hover:text-deep-purple-accent-400'
                            aria-label='Login'
                            title='Login'
                          >
                            Login
                          </a>
                        </li>
                      )}
                    </ul>
                  </nav>
                </div>
              </div>
            )}
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navigation;
