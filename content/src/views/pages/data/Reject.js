...
const Reject = () => {
  ...
  const getData = async () => {
    try {
      const {data} = await axios({
        method: 'GET',
        url: `${HostUrl}/claim?status=reject`,
        headers: {
  ...
  const handdleEdit = (claimId, userId) => {
  ...
  const fields = [
    ...
    {key: 'updated_date', label: 'Tanggal Reject', sorter: true, filter: true},
        ...
        <CCol>
          <CCard>
            <CCardHeader color='success'>
              <strong>Pengajuan Claim Reject</strong>
            </CCardHeader>
            {dataPeserta && (
              <CCardBody>
                <CDataTable
